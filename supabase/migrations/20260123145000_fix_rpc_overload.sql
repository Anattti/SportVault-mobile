-- Migration: Fix RPC Overloading
-- Description: Drops conflicting versions of insert_workout_with_children and recreates the authoritative version.

-- Drop potential conflicting signatures based on the error message
DROP FUNCTION IF EXISTS public.insert_workout_with_children(uuid, text, text, timestamp with time zone, integer, integer, text, text, text, jsonb);
DROP FUNCTION IF EXISTS public.insert_workout_with_children(uuid, text, text, timestamp with time zone, integer, integer, jsonb, text, text, text);

-- Update the exercises table again just to be sure (idempotent)
    ...
    loop
      ...
      if jsonb_array_length(v_exercise_item->'sets') > 0 then
        ...
        select
          ...
          coalesce((set_item->>'rest_time')::int, (set_item->>'restTime')::int, 60),
          (set_item->>'rpe')::numeric,
          coalesce((set_item->>'isBodyweight')::boolean, (set_item->>'weight')::numeric = 0),
