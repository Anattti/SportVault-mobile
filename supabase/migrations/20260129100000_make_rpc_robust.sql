-- Migration: Make save_full_workout_session robust against missing exercises
-- Description: Updates the RPC to handle cases where an exercise referenced in the session
-- has been deleted from the template. In such cases, exercise_id is set to NULL,
-- but the session data (name, sets, etc.) is preserved.
...
begin
  ...
  if jsonb_array_length(v_exercises) > 0 then
    ...
    select
      ...
      -- Safety check: Only insert exercise_id if it exists in the exercises table
      CASE 
        WHEN EXISTS (SELECT 1 FROM public.exercises WHERE id = (item->>'exercise_id')::uuid) 
  ...
  WITH session_stats AS (
      ...
      AND se.exercise_id IS NOT NULL -- Only update if linked to template
