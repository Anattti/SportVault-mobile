-- Create RPC function to save full workout session atomically
-- This function handles upserts for workout_sessions, session_exercises, and session_sets
...
declare
  ...
  v_sets jsonb;
begin
  -- payload structure: { session: {...}, exercises: [...], sets: [...] }
  ...
  if jsonb_array_length(v_sets) > 0 then
    insert into public.session_sets (
      id, session_exercise_id, sets_completed, reps_completed, weight_used, rpe, rest_time_taken, created_at, completed_at, _offline, _pendingsync
    )
    select
      (item->>'id')::uuid,
