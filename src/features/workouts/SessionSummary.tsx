...
export function SessionSummary({ sessionId: propSessionId, onClose }: SessionSummaryProps) {
            ...
            {session?.warmup && session.warmup.duration > 0 && (
              <View style={[styles.badgeContainer, { flex: 1 }]}>
                <View style={styles.badge}>
                  ...
                  {/* @ts-ignore */}
                  <Text style={styles.badgeText}>{formatDuration(session.warmup.duration)} min</Text>
                ...
                {session?.warmup?.notes && (
                  <Text style={styles.badgeNoteText} numberOfLines={2}>
                    {/* @ts-ignore */}
                    "{session.warmup.notes}"
                  </Text>
                )}
            ...
            {session?.cooldown && session.cooldown.duration > 0 && (
              <View style={[styles.badgeContainer, { flex: 1 }]}>
                <View style={styles.badge}>
                  ...
                  {/* @ts-ignore */}
                  <Text style={styles.badgeText}>{formatDuration(session.cooldown.duration)} min</Text>
                  <View style={styles.badgeDivider} />
                  {/* @ts-ignore */}
                  <Text style={styles.badgeSubText}>{(session.cooldown.method || 'COOLDOWN').toUpperCase()}</Text>
                </View>
                {/* @ts-ignore */}
                {session?.cooldown?.notes && (
                  <Text style={styles.badgeNoteText} numberOfLines={2}>
                    {/* @ts-ignore */}
                    "{session.cooldown.notes}"
                  </Text>
